...
- `--seq`: путь к последовательности, для которой необходимо предсказать путь состояний. Например - `hw1/test/test_seq` (есть в проекте - можете протестировать). **Замените на свою последовательность, если интересуетесь корректностью алгоритма**. 
- `--output`: путь для сохранения предсказанного пути состояний.
- `--config`: путь к конфигурационному файлу (уже есть в папке проекта по указанному в команде пути; при необходимости можете изменить параметры).
